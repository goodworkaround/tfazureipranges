output "ActionGroup_region32_ipv4" {
  value = [
    "20.39.53.174/32",
    "20.42.64.36/30",
    "40.121.219.215/32",
    "40.121.221.62/32",
    "40.121.222.201/32",
    "40.121.223.186/32",
    "52.149.154.142/32",
    "52.154.76.93/32",
    "52.154.77.164/32",
    "52.188.200.146/32",
    "104.211.0.27/32",
    "104.211.2.38/32",
    "104.211.3.34/32",
    "104.211.3.100/32",
    "137.117.45.230/32",
    "137.117.46.62/32",
    "137.117.46.248/32",
    "168.61.46.64/32",
    "168.61.47.22/32",
  ]
}
output "ActionGroup_region32_ipv6" {
  value = [
    "2603:1030:210:402::178/125",
  ]
}
output "ActionGroup_region32" {
  value = [
    "20.39.53.174/32",
    "20.42.64.36/30",
    "40.121.219.215/32",
    "40.121.221.62/32",
    "40.121.222.201/32",
    "40.121.223.186/32",
    "52.149.154.142/32",
    "52.154.76.93/32",
    "52.154.77.164/32",
    "52.188.200.146/32",
    "104.211.0.27/32",
    "104.211.2.38/32",
    "104.211.3.34/32",
    "104.211.3.100/32",
    "137.117.45.230/32",
    "137.117.46.62/32",
    "137.117.46.248/32",
    "168.61.46.64/32",
    "168.61.47.22/32",
    "2603:1030:210:402::178/125",
  ]
}
output "AzureApiManagement_region32_ipv4" {
  value = [
    "20.88.154.20/31",
    "40.71.10.204/31",
    "40.71.13.128/28",
    "52.224.186.99/32",
  ]
}
output "AzureApiManagement_region32_ipv6" {
  value = [
    "2603:1030:210:402::140/124",
  ]
}
output "AzureApiManagement_region32" {
  value = [
    "20.88.154.20/31",
    "40.71.10.204/31",
    "40.71.13.128/28",
    "52.224.186.99/32",
    "2603:1030:210:402::140/124",
  ]
}
output "AzureAppService_region32_ipv4" {
  value = [
    "13.82.93.245/32",
    "13.82.101.179/32",
    "13.82.175.96/32",
    "13.90.143.69/32",
    "13.90.213.204/32",
    "13.92.139.214/32",
    "13.92.193.110/32",
    "13.92.237.218/32",
    "20.42.26.252/32",
    "20.49.104.0/25",
    "20.119.0.0/20",
    "20.119.16.0/21",
    "20.119.24.0/23",
    "20.119.26.0/24",
    "20.119.27.0/25",
    "23.96.0.52/32",
    "23.96.1.109/32",
    "23.96.13.243/32",
    "23.96.32.128/32",
    "23.96.96.142/32",
    "23.96.103.159/32",
    "23.96.112.53/32",
    "23.96.113.128/32",
    "23.96.124.25/32",
    "40.71.0.179/32",
    "40.71.11.128/25",
    "40.71.177.34/32",
    "40.71.199.117/32",
    "40.71.234.254/32",
    "40.71.250.191/32",
    "40.76.5.137/32",
    "40.76.192.15/32",
    "40.76.210.54/32",
    "40.76.218.33/32",
    "40.76.223.101/32",
    "40.79.154.192/27",
    "40.85.190.10/32",
    "40.87.65.131/32",
    "40.87.70.95/32",
    "40.114.13.25/32",
    "40.114.41.245/32",
    "40.114.51.68/32",
    "40.114.68.21/32",
    "40.114.106.25/32",
    "40.117.154.240/32",
    "40.117.188.126/32",
    "40.117.190.72/32",
    "40.121.8.241/32",
    "40.121.16.193/32",
    "40.121.32.232/32",
    "40.121.35.221/32",
    "40.121.91.199/32",
    "40.121.212.165/32",
    "40.121.221.52/32",
    "52.168.125.188/32",
    "52.170.7.25/32",
    "52.170.46.174/32",
    "52.179.97.15/32",
    "52.226.134.64/32",
    "52.234.209.94/32",
    "104.45.129.178/32",
    "104.45.141.247/32",
    "104.45.152.13/32",
    "104.45.152.60/32",
    "104.45.154.200/32",
    "104.211.26.212/32",
    "137.117.58.204/32",
    "137.117.66.167/32",
    "137.117.84.54/32",
    "137.117.90.63/32",
    "137.117.93.87/32",
    "137.135.91.176/32",
    "137.135.107.235/32",
    "168.62.48.183/32",
    "168.62.180.173/32",
    "191.236.16.12/32",
    "191.236.59.67/32",
    "191.237.24.89/32",
    "191.237.27.74/32",
    "191.238.8.26/32",
    "191.238.33.50/32",
  ]
}
output "AzureAppService_region32_ipv6" {
  value = [
    "2603:1030:210:6::/117",
    "2603:1030:210:7::/117",
    "2603:1030:210:8::/117",
    "2603:1030:210:9::/117",
    "2603:1030:210:402::a0/123",
    "2603:1030:210:802::a0/123",
    "2603:1030:210:c02::a0/123",
  ]
}
output "AzureAppService_region32" {
  value = [
    "13.82.93.245/32",
    "13.82.101.179/32",
    "13.82.175.96/32",
    "13.90.143.69/32",
    "13.90.213.204/32",
    "13.92.139.214/32",
    "13.92.193.110/32",
    "13.92.237.218/32",
    "20.42.26.252/32",
    "20.49.104.0/25",
    "20.119.0.0/20",
    "20.119.16.0/21",
    "20.119.24.0/23",
    "20.119.26.0/24",
    "20.119.27.0/25",
    "23.96.0.52/32",
    "23.96.1.109/32",
    "23.96.13.243/32",
    "23.96.32.128/32",
    "23.96.96.142/32",
    "23.96.103.159/32",
    "23.96.112.53/32",
    "23.96.113.128/32",
    "23.96.124.25/32",
    "40.71.0.179/32",
    "40.71.11.128/25",
    "40.71.177.34/32",
    "40.71.199.117/32",
    "40.71.234.254/32",
    "40.71.250.191/32",
    "40.76.5.137/32",
    "40.76.192.15/32",
    "40.76.210.54/32",
    "40.76.218.33/32",
    "40.76.223.101/32",
    "40.79.154.192/27",
    "40.85.190.10/32",
    "40.87.65.131/32",
    "40.87.70.95/32",
    "40.114.13.25/32",
    "40.114.41.245/32",
    "40.114.51.68/32",
    "40.114.68.21/32",
    "40.114.106.25/32",
    "40.117.154.240/32",
    "40.117.188.126/32",
    "40.117.190.72/32",
    "40.121.8.241/32",
    "40.121.16.193/32",
    "40.121.32.232/32",
    "40.121.35.221/32",
    "40.121.91.199/32",
    "40.121.212.165/32",
    "40.121.221.52/32",
    "52.168.125.188/32",
    "52.170.7.25/32",
    "52.170.46.174/32",
    "52.179.97.15/32",
    "52.226.134.64/32",
    "52.234.209.94/32",
    "104.45.129.178/32",
    "104.45.141.247/32",
    "104.45.152.13/32",
    "104.45.152.60/32",
    "104.45.154.200/32",
    "104.211.26.212/32",
    "137.117.58.204/32",
    "137.117.66.167/32",
    "137.117.84.54/32",
    "137.117.90.63/32",
    "137.117.93.87/32",
    "137.135.91.176/32",
    "137.135.107.235/32",
    "168.62.48.183/32",
    "168.62.180.173/32",
    "191.236.16.12/32",
    "191.236.59.67/32",
    "191.237.24.89/32",
    "191.237.27.74/32",
    "191.238.8.26/32",
    "191.238.33.50/32",
    "2603:1030:210:6::/117",
    "2603:1030:210:7::/117",
    "2603:1030:210:8::/117",
    "2603:1030:210:9::/117",
    "2603:1030:210:402::a0/123",
    "2603:1030:210:802::a0/123",
    "2603:1030:210:c02::a0/123",
  ]
}
output "AzureAppServiceManagement_region32_ipv4" {
  value = [
    "20.42.68.128/26",
    "20.42.74.128/26",
    "20.88.157.128/28",
    "40.71.11.0/26",
    "40.71.13.64/26",
    "52.224.105.172/32",
    "191.236.60.72/32",
  ]
}
output "AzureAppServiceManagement_region32_ipv6" {
  value = [
    "2603:1030:210::7a0/123",
    "2603:1030:210:402::100/122",
    "2603:1030:210:802::100/122",
    "2603:1030:210:c02::100/122",
  ]
}
output "AzureAppServiceManagement_region32" {
  value = [
    "20.42.68.128/26",
    "20.42.74.128/26",
    "20.88.157.128/28",
    "40.71.11.0/26",
    "40.71.13.64/26",
    "52.224.105.172/32",
    "191.236.60.72/32",
    "2603:1030:210::7a0/123",
    "2603:1030:210:402::100/122",
    "2603:1030:210:802::100/122",
    "2603:1030:210:c02::100/122",
  ]
}
output "AzureArcInfrastructure_region32_ipv4" {
  value = [
    "13.90.194.180/32",
    "20.42.74.230/32",
    "20.42.74.232/30",
    "20.49.109.32/30",
    "40.71.15.194/32",
    "52.146.79.132/30",
    "137.135.98.137/32",
  ]
}
output "AzureArcInfrastructure_region32_ipv6" {
  value = [
  ]
}
output "AzureArcInfrastructure_region32" {
  value = [
    "13.90.194.180/32",
    "20.42.74.230/32",
    "20.42.74.232/30",
    "20.49.109.32/30",
    "40.71.15.194/32",
    "52.146.79.132/30",
    "137.135.98.137/32",
  ]
}
output "AzureAttestation_region32_ipv4" {
  value = [
    "20.62.129.148/30",
    "20.72.129.122/32",
    "40.71.15.204/30",
    "40.88.132.87/32",
  ]
}
output "AzureAttestation_region32_ipv6" {
  value = [
  ]
}
output "AzureAttestation_region32" {
  value = [
    "20.62.129.148/30",
    "20.72.129.122/32",
    "40.71.15.204/30",
    "40.88.132.87/32",
  ]
}
output "AzureBackup_region32_ipv4" {
  value = [
    "20.62.133.128/25",
    "20.62.134.0/26",
    "40.71.12.0/25",
    "40.71.12.128/26",
    "40.78.227.64/26",
    "40.78.227.128/25",
    "40.79.155.64/26",
    "40.79.155.128/25",
  ]
}
output "AzureBackup_region32_ipv6" {
  value = [
    "2603:1030:210:402::200/121",
    "2603:1030:210:802::180/121",
    "2603:1030:210:c02::180/121",
  ]
}
output "AzureBackup_region32" {
  value = [
    "20.62.133.128/25",
    "20.62.134.0/26",
    "40.71.12.0/25",
    "40.71.12.128/26",
    "40.78.227.64/26",
    "40.78.227.128/25",
    "40.79.155.64/26",
    "40.79.155.128/25",
    "2603:1030:210:402::200/121",
    "2603:1030:210:802::180/121",
    "2603:1030:210:c02::180/121",
  ]
}
output "AzureBotService_region32_ipv4" {
  value = [
    "20.42.0.64/30",
    "40.71.12.244/30",
  ]
}
output "AzureBotService_region32_ipv6" {
  value = [
    "2603:1030:210:1::20/123",
  ]
}
output "AzureBotService_region32" {
  value = [
    "20.42.0.64/30",
    "40.71.12.244/30",
    "2603:1030:210:1::20/123",
  ]
}
output "region32_ipv4" {
  value = [
    "13.68.128.0/17",
    "13.72.64.0/18",
    "13.82.0.0/16",
    "13.90.0.0/16",
    "13.92.0.0/16",
    "13.104.144.128/27",
    "13.104.152.128/25",
    "13.104.192.0/21",
    "13.104.211.0/25",
    "13.104.214.128/25",
    "13.104.215.0/25",
    "13.105.17.0/26",
    "13.105.19.0/25",
    "13.105.20.192/26",
    "13.105.27.0/25",
    "13.105.27.192/27",
    "13.105.36.192/26",
    "13.105.74.48/28",
    "13.105.98.48/28",
    "13.105.98.96/27",
    "13.105.98.128/27",
    "20.25.0.0/17",
    "20.38.98.0/24",
    "20.39.32.0/19",
    "20.42.0.0/17",
    "20.47.1.0/24",
    "20.47.16.0/23",
    "20.47.31.0/24",
    "20.47.108.0/23",
    "20.47.113.0/24",
    "20.49.104.0/21",
    "20.51.128.0/17",
    "20.55.0.0/17",
    "20.60.0.0/24",
    "20.60.2.0/23",
    "20.60.6.0/23",
    "20.60.60.0/22",
    "20.60.128.0/23",
    "20.60.134.0/23",
    "20.60.146.0/23",
    "20.60.220.0/23",
    "20.62.128.0/17",
    "20.72.128.0/18",
    "20.75.128.0/17",
    "20.81.0.0/17",
    "20.83.128.0/18",
    "20.84.0.0/17",
    "20.85.128.0/17",
    "20.88.128.0/18",
    "20.95.0.0/21",
    "20.95.32.0/21",
    "20.102.0.0/17",
    "20.106.128.0/17",
    "20.115.0.0/17",
    "20.119.0.0/17",
    "20.120.0.0/17",
    "20.121.0.0/16",
    "20.124.0.0/16",
    "20.127.0.0/16",
    "20.135.4.0/23",
    "20.135.194.0/23",
    "20.135.196.0/22",
    "20.150.32.0/23",
    "20.150.90.0/24",
    "20.157.6.0/23",
    "20.157.19.0/24",
    "20.157.39.0/24",
    "20.157.59.0/24",
    "20.157.61.0/24",
    "20.157.104.0/24",
    "20.157.132.0/24",
    "20.157.147.0/24",
    "20.185.0.0/16",
    "20.190.130.0/24",
    "20.190.151.0/24",
    "20.202.20.0/24",
    "20.202.39.0/24",
    "20.202.120.0/22",
    "20.202.124.0/24",
    "20.209.0.0/23",
    "23.96.0.0/17",
    "23.98.45.0/24",
    "23.100.16.0/20",
    "23.101.128.0/20",
    "40.71.0.0/16",
    "40.76.0.0/16",
    "40.78.219.0/24",
    "40.78.224.0/21",
    "40.79.152.0/21",
    "40.80.144.0/21",
    "40.82.24.0/22",
    "40.82.60.0/22",
    "40.85.160.0/19",
    "40.87.0.0/17",
    "40.87.164.0/22",
    "40.88.0.0/16",
    "40.90.23.128/25",
    "40.90.24.128/25",
    "40.90.25.0/26",
    "40.90.30.192/26",
    "40.90.129.128/26",
    "40.90.130.96/28",
    "40.90.131.224/27",
    "40.90.136.16/28",
    "40.90.136.32/27",
    "40.90.137.96/27",
    "40.90.139.224/27",
    "40.90.143.0/27",
    "40.90.146.64/26",
    "40.90.147.0/27",
    "40.90.148.64/27",
    "40.90.150.32/27",
    "40.90.224.0/19",
    "40.91.4.0/22",
    "40.93.2.0/24",
    "40.93.4.0/24",
    "40.93.11.0/24",
    "40.112.48.0/20",
    "40.114.0.0/17",
    "40.117.32.0/19",
    "40.117.64.0/18",
    "40.117.128.0/17",
    "40.121.0.0/16",
    "40.123.132.0/22",
    "40.126.2.0/24",
    "40.126.23.0/24",
    "52.101.4.0/22",
    "52.101.9.0/24",
    "52.101.20.0/22",
    "52.101.51.0/24",
    "52.101.52.0/22",
    "52.102.129.0/24",
    "52.102.137.0/24",
    "52.102.159.0/24",
    "52.103.1.0/24",
    "52.103.3.0/24",
    "52.103.11.0/24",
    "52.103.129.0/24",
    "52.103.137.0/24",
    "52.108.16.0/21",
    "52.108.79.0/24",
    "52.108.105.0/24",
    "52.108.106.0/23",
    "52.109.12.0/22",
    "52.111.229.0/24",
    "52.112.112.0/24",
    "52.113.16.0/20",
    "52.114.132.0/22",
    "52.115.54.0/24",
    "52.115.62.0/23",
    "52.115.192.0/19",
    "52.120.32.0/19",
    "52.120.224.0/20",
    "52.122.0.0/24",
    "52.123.0.0/24",
    "52.125.132.0/22",
    "52.136.64.0/18",
    "52.142.0.0/18",
    "52.143.207.0/24",
    "52.146.0.0/17",
    "52.147.192.0/18",
    "52.149.128.0/17",
    "52.150.0.0/17",
    "52.151.128.0/17",
    "52.152.128.0/17",
    "52.154.64.0/18",
    "52.168.0.0/16",
    "52.170.0.0/16",
    "52.179.0.0/17",
    "52.186.0.0/16",
    "52.188.0.0/16",
    "52.190.0.0/17",
    "52.191.0.0/17",
    "52.191.192.0/18",
    "52.224.0.0/16",
    "52.226.0.0/16",
    "52.232.146.0/24",
    "52.234.128.0/17",
    "52.239.152.0/22",
    "52.239.168.0/22",
    "52.239.207.192/26",
    "52.239.214.0/23",
    "52.239.220.0/23",
    "52.239.246.0/23",
    "52.239.252.0/24",
    "52.240.0.0/17",
    "52.245.8.0/22",
    "52.245.104.0/22",
    "52.249.128.0/17",
    "52.253.160.0/24",
    "52.255.128.0/17",
    "53.103.137.0/24",
    "65.54.19.128/27",
    "104.41.128.0/19",
    "104.44.91.32/27",
    "104.44.94.16/28",
    "104.44.95.160/27",
    "104.44.95.240/28",
    "104.45.128.0/18",
    "104.45.192.0/20",
    "104.211.0.0/18",
    "137.116.112.0/20",
    "137.117.32.0/19",
    "137.117.64.0/18",
    "137.135.64.0/18",
    "138.91.96.0/19",
    "157.56.176.0/21",
    "168.61.32.0/20",
    "168.61.48.0/21",
    "168.62.32.0/19",
    "168.62.160.0/19",
    "191.234.32.0/19",
    "191.236.0.0/18",
    "191.237.0.0/17",
    "191.238.0.0/18",
    "204.152.18.0/31",
    "204.152.18.8/29",
    "204.152.18.32/27",
    "204.152.18.64/26",
    "204.152.19.0/24",
  ]
}
output "region32_ipv6" {
  value = [
    "2602:fd5e:1::/63",
    "2602:fd5e:1:2::/64",
    "2603:1030:20c::/47",
    "2603:1030:20e::/48",
    "2603:1030:210::/47",
    "2603:1030:212::/56",
    "2603:1030:213::/48",
    "2603:1036:120d::/48",
    "2603:1036:2404::/48",
    "2603:1036:3000:120::/59",
    "2603:1037:1:120::/59",
    "2a01:111:f100:2000::/52",
    "2a01:111:f403:c100::/64",
    "2a01:111:f403:c900::/64",
    "2a01:111:f403:c91e::/63",
    "2a01:111:f403:c920::/63",
    "2a01:111:f403:c922::/64",
    "2a01:111:f403:d100::/64",
    "2a01:111:f403:d900::/64",
    "2a01:111:f403:f000::/64",
    "2a01:111:f403:f900::/62",
  ]
}
output "region32" {
  value = [
    "13.68.128.0/17",
    "13.72.64.0/18",
    "13.82.0.0/16",
    "13.90.0.0/16",
    "13.92.0.0/16",
    "13.104.144.128/27",
    "13.104.152.128/25",
    "13.104.192.0/21",
    "13.104.211.0/25",
    "13.104.214.128/25",
    "13.104.215.0/25",
    "13.105.17.0/26",
    "13.105.19.0/25",
    "13.105.20.192/26",
    "13.105.27.0/25",
    "13.105.27.192/27",
    "13.105.36.192/26",
    "13.105.74.48/28",
    "13.105.98.48/28",
    "13.105.98.96/27",
    "13.105.98.128/27",
    "20.25.0.0/17",
    "20.38.98.0/24",
    "20.39.32.0/19",
    "20.42.0.0/17",
    "20.47.1.0/24",
    "20.47.16.0/23",
    "20.47.31.0/24",
    "20.47.108.0/23",
    "20.47.113.0/24",
    "20.49.104.0/21",
    "20.51.128.0/17",
    "20.55.0.0/17",
    "20.60.0.0/24",
    "20.60.2.0/23",
    "20.60.6.0/23",
    "20.60.60.0/22",
    "20.60.128.0/23",
    "20.60.134.0/23",
    "20.60.146.0/23",
    "20.60.220.0/23",
    "20.62.128.0/17",
    "20.72.128.0/18",
    "20.75.128.0/17",
    "20.81.0.0/17",
    "20.83.128.0/18",
    "20.84.0.0/17",
    "20.85.128.0/17",
    "20.88.128.0/18",
    "20.95.0.0/21",
    "20.95.32.0/21",
    "20.102.0.0/17",
    "20.106.128.0/17",
    "20.115.0.0/17",
    "20.119.0.0/17",
    "20.120.0.0/17",
    "20.121.0.0/16",
    "20.124.0.0/16",
    "20.127.0.0/16",
    "20.135.4.0/23",
    "20.135.194.0/23",
    "20.135.196.0/22",
    "20.150.32.0/23",
    "20.150.90.0/24",
    "20.157.6.0/23",
    "20.157.19.0/24",
    "20.157.39.0/24",
    "20.157.59.0/24",
    "20.157.61.0/24",
    "20.157.104.0/24",
    "20.157.132.0/24",
    "20.157.147.0/24",
    "20.185.0.0/16",
    "20.190.130.0/24",
    "20.190.151.0/24",
    "20.202.20.0/24",
    "20.202.39.0/24",
    "20.202.120.0/22",
    "20.202.124.0/24",
    "20.209.0.0/23",
    "23.96.0.0/17",
    "23.98.45.0/24",
    "23.100.16.0/20",
    "23.101.128.0/20",
    "40.71.0.0/16",
    "40.76.0.0/16",
    "40.78.219.0/24",
    "40.78.224.0/21",
    "40.79.152.0/21",
    "40.80.144.0/21",
    "40.82.24.0/22",
    "40.82.60.0/22",
    "40.85.160.0/19",
    "40.87.0.0/17",
    "40.87.164.0/22",
    "40.88.0.0/16",
    "40.90.23.128/25",
    "40.90.24.128/25",
    "40.90.25.0/26",
    "40.90.30.192/26",
    "40.90.129.128/26",
    "40.90.130.96/28",
    "40.90.131.224/27",
    "40.90.136.16/28",
    "40.90.136.32/27",
    "40.90.137.96/27",
    "40.90.139.224/27",
    "40.90.143.0/27",
    "40.90.146.64/26",
    "40.90.147.0/27",
    "40.90.148.64/27",
    "40.90.150.32/27",
    "40.90.224.0/19",
    "40.91.4.0/22",
    "40.93.2.0/24",
    "40.93.4.0/24",
    "40.93.11.0/24",
    "40.112.48.0/20",
    "40.114.0.0/17",
    "40.117.32.0/19",
    "40.117.64.0/18",
    "40.117.128.0/17",
    "40.121.0.0/16",
    "40.123.132.0/22",
    "40.126.2.0/24",
    "40.126.23.0/24",
    "52.101.4.0/22",
    "52.101.9.0/24",
    "52.101.20.0/22",
    "52.101.51.0/24",
    "52.101.52.0/22",
    "52.102.129.0/24",
    "52.102.137.0/24",
    "52.102.159.0/24",
    "52.103.1.0/24",
    "52.103.3.0/24",
    "52.103.11.0/24",
    "52.103.129.0/24",
    "52.103.137.0/24",
    "52.108.16.0/21",
    "52.108.79.0/24",
    "52.108.105.0/24",
    "52.108.106.0/23",
    "52.109.12.0/22",
    "52.111.229.0/24",
    "52.112.112.0/24",
    "52.113.16.0/20",
    "52.114.132.0/22",
    "52.115.54.0/24",
    "52.115.62.0/23",
    "52.115.192.0/19",
    "52.120.32.0/19",
    "52.120.224.0/20",
    "52.122.0.0/24",
    "52.123.0.0/24",
    "52.125.132.0/22",
    "52.136.64.0/18",
    "52.142.0.0/18",
    "52.143.207.0/24",
    "52.146.0.0/17",
    "52.147.192.0/18",
    "52.149.128.0/17",
    "52.150.0.0/17",
    "52.151.128.0/17",
    "52.152.128.0/17",
    "52.154.64.0/18",
    "52.168.0.0/16",
    "52.170.0.0/16",
    "52.179.0.0/17",
    "52.186.0.0/16",
    "52.188.0.0/16",
    "52.190.0.0/17",
    "52.191.0.0/17",
    "52.191.192.0/18",
    "52.224.0.0/16",
    "52.226.0.0/16",
    "52.232.146.0/24",
    "52.234.128.0/17",
    "52.239.152.0/22",
    "52.239.168.0/22",
    "52.239.207.192/26",
    "52.239.214.0/23",
    "52.239.220.0/23",
    "52.239.246.0/23",
    "52.239.252.0/24",
    "52.240.0.0/17",
    "52.245.8.0/22",
    "52.245.104.0/22",
    "52.249.128.0/17",
    "52.253.160.0/24",
    "52.255.128.0/17",
    "53.103.137.0/24",
    "65.54.19.128/27",
    "104.41.128.0/19",
    "104.44.91.32/27",
    "104.44.94.16/28",
    "104.44.95.160/27",
    "104.44.95.240/28",
    "104.45.128.0/18",
    "104.45.192.0/20",
    "104.211.0.0/18",
    "137.116.112.0/20",
    "137.117.32.0/19",
    "137.117.64.0/18",
    "137.135.64.0/18",
    "138.91.96.0/19",
    "157.56.176.0/21",
    "168.61.32.0/20",
    "168.61.48.0/21",
    "168.62.32.0/19",
    "168.62.160.0/19",
    "191.234.32.0/19",
    "191.236.0.0/18",
    "191.237.0.0/17",
    "191.238.0.0/18",
    "204.152.18.0/31",
    "204.152.18.8/29",
    "204.152.18.32/27",
    "204.152.18.64/26",
    "204.152.19.0/24",
    "2602:fd5e:1::/63",
    "2602:fd5e:1:2::/64",
    "2603:1030:20c::/47",
    "2603:1030:20e::/48",
    "2603:1030:210::/47",
    "2603:1030:212::/56",
    "2603:1030:213::/48",
    "2603:1036:120d::/48",
    "2603:1036:2404::/48",
    "2603:1036:3000:120::/59",
    "2603:1037:1:120::/59",
    "2a01:111:f100:2000::/52",
    "2a01:111:f403:c100::/64",
    "2a01:111:f403:c900::/64",
    "2a01:111:f403:c91e::/63",
    "2a01:111:f403:c920::/63",
    "2a01:111:f403:c922::/64",
    "2a01:111:f403:d100::/64",
    "2a01:111:f403:d900::/64",
    "2a01:111:f403:f000::/64",
    "2a01:111:f403:f900::/62",
  ]
}
output "AzureCognitiveSearch_region32_ipv4" {
  value = [
    "20.42.4.128/26",
    "20.42.24.90/32",
    "20.42.29.212/32",
    "20.42.30.105/32",
    "20.42.34.190/32",
    "20.42.35.204/32",
    "20.185.110.199/32",
    "40.90.240.17/32",
    "52.151.235.150/32",
    "52.151.235.242/32",
    "52.151.235.244/32",
    "52.188.217.235/32",
    "52.188.218.228/32",
    "52.188.218.239/32",
  ]
}
output "AzureCognitiveSearch_region32_ipv6" {
  value = [
    "2603:1030:210:1::180/121",
  ]
}
output "AzureCognitiveSearch_region32" {
  value = [
    "20.42.4.128/26",
    "20.42.24.90/32",
    "20.42.29.212/32",
    "20.42.30.105/32",
    "20.42.34.190/32",
    "20.42.35.204/32",
    "20.185.110.199/32",
    "40.90.240.17/32",
    "52.151.235.150/32",
    "52.151.235.242/32",
    "52.151.235.244/32",
    "52.188.217.235/32",
    "52.188.218.228/32",
    "52.188.218.239/32",
    "2603:1030:210:1::180/121",
  ]
}
output "AzureConnectors_region32_ipv4" {
  value = [
    "20.88.153.176/28",
    "20.88.153.192/27",
    "40.71.11.80/28",
    "40.71.15.160/27",
    "40.71.249.139/32",
    "40.71.249.205/32",
    "40.114.40.132/32",
  ]
}
output "AzureConnectors_region32_ipv6" {
  value = [
    "2603:1030:210:402::180/122",
  ]
}
output "AzureConnectors_region32" {
  value = [
    "20.88.153.176/28",
    "20.88.153.192/27",
    "40.71.11.80/28",
    "40.71.15.160/27",
    "40.71.249.139/32",
    "40.71.249.205/32",
    "40.114.40.132/32",
    "2603:1030:210:402::180/122",
  ]
}
output "AzureContainerRegistry_region32_ipv4" {
  value = [
    "20.42.66.0/24",
    "20.42.67.0/24",
    "20.42.74.64/26",
    "20.62.128.0/26",
    "40.71.10.216/29",
    "40.78.226.208/29",
    "40.78.231.0/24",
    "40.79.154.104/29",
    "52.168.112.192/26",
    "52.168.114.0/24",
    "52.168.115.0/24",
  ]
}
output "AzureContainerRegistry_region32_ipv6" {
  value = [
    "2603:1030:210:402::90/125",
    "2603:1030:210:402::340/122",
    "2603:1030:210:402::580/121",
    "2603:1030:210:802::90/125",
    "2603:1030:210:802::2c0/122",
    "2603:1030:210:802::400/121",
    "2603:1030:210:c02::90/125",
    "2603:1030:210:c02::400/121",
  ]
}
output "AzureContainerRegistry_region32" {
  value = [
    "20.42.66.0/24",
    "20.42.67.0/24",
    "20.42.74.64/26",
    "20.62.128.0/26",
    "40.71.10.216/29",
    "40.78.226.208/29",
    "40.78.231.0/24",
    "40.79.154.104/29",
    "52.168.112.192/26",
    "52.168.114.0/24",
    "52.168.115.0/24",
    "2603:1030:210:402::90/125",
    "2603:1030:210:402::340/122",
    "2603:1030:210:402::580/121",
    "2603:1030:210:802::90/125",
    "2603:1030:210:802::2c0/122",
    "2603:1030:210:802::400/121",
    "2603:1030:210:c02::90/125",
    "2603:1030:210:c02::400/121",
  ]
}
output "AzureCosmosDB_region32_ipv4" {
  value = [
    "13.82.53.191/32",
    "13.90.199.155/32",
    "40.71.10.0/25",
    "40.71.17.19/32",
    "40.71.203.37/32",
    "40.71.204.115/32",
    "40.71.216.114/32",
    "40.78.226.0/25",
    "40.79.154.128/26",
    "40.85.178.211/32",
    "52.146.79.160/27",
    "52.168.28.222/32",
    "52.170.204.83/32",
    "52.186.69.224/32",
    "52.191.197.220/32",
    "52.226.18.140/32",
    "52.226.21.178/32",
    "104.45.131.193/32",
    "104.45.144.73/32",
  ]
}
output "AzureCosmosDB_region32_ipv6" {
  value = [
    "2603:1030:210:402::c0/122",
    "2603:1030:210:802::c0/122",
    "2603:1030:210:c02::c0/122",
  ]
}
output "AzureCosmosDB_region32" {
  value = [
    "13.82.53.191/32",
    "13.90.199.155/32",
    "40.71.10.0/25",
    "40.71.17.19/32",
    "40.71.203.37/32",
    "40.71.204.115/32",
    "40.71.216.114/32",
    "40.78.226.0/25",
    "40.79.154.128/26",
    "40.85.178.211/32",
    "52.146.79.160/27",
    "52.168.28.222/32",
    "52.170.204.83/32",
    "52.186.69.224/32",
    "52.191.197.220/32",
    "52.226.18.140/32",
    "52.226.21.178/32",
    "104.45.131.193/32",
    "104.45.144.73/32",
    "2603:1030:210:402::c0/122",
    "2603:1030:210:802::c0/122",
    "2603:1030:210:c02::c0/122",
  ]
}
output "AzureDataExplorerManagement_region32_ipv4" {
  value = [
    "20.185.100.27/32",
    "40.71.13.176/28",
    "52.224.146.56/32",
  ]
}
output "AzureDataExplorerManagement_region32_ipv6" {
  value = [
    "2603:1030:210::600/121",
    "2603:1030:210:402::150/124",
  ]
}
output "AzureDataExplorerManagement_region32" {
  value = [
    "20.185.100.27/32",
    "40.71.13.176/28",
    "52.224.146.56/32",
    "2603:1030:210::600/121",
    "2603:1030:210:402::150/124",
  ]
}
output "AzureDevOps_region32_ipv4" {
  value = [
    "20.42.5.0/24",
  ]
}
output "AzureDevOps_region32_ipv6" {
  value = [
  ]
}
output "AzureDevOps_region32" {
  value = [
    "20.42.5.0/24",
  ]
}
output "AzureDevSpaces_region32_ipv4" {
  value = [
    "20.42.6.32/27",
    "20.42.6.128/28",
    "20.42.64.64/26",
  ]
}
output "AzureDevSpaces_region32_ipv6" {
  value = [
  ]
}
output "AzureDevSpaces_region32" {
  value = [
    "20.42.6.32/27",
    "20.42.6.128/28",
    "20.42.64.64/26",
  ]
}
output "AzureDigitalTwins_region32_ipv4" {
  value = [
    "20.62.129.32/27",
    "20.62.129.128/29",
    "20.185.75.6/32",
    "20.185.75.209/32",
    "52.149.234.152/32",
    "52.149.238.190/32",
    "52.149.239.34/32",
    "52.170.161.49/32",
    "52.170.162.28/32",
    "52.186.106.218/32",
    "52.191.16.191/32",
    "52.191.18.106/32",
  ]
}
output "AzureDigitalTwins_region32_ipv6" {
  value = [
  ]
}
output "AzureDigitalTwins_region32" {
  value = [
    "20.62.129.32/27",
    "20.62.129.128/29",
    "20.185.75.6/32",
    "20.185.75.209/32",
    "52.149.234.152/32",
    "52.149.238.190/32",
    "52.149.239.34/32",
    "52.170.161.49/32",
    "52.170.162.28/32",
    "52.186.106.218/32",
    "52.191.16.191/32",
    "52.191.18.106/32",
  ]
}
output "AzureEventGrid_region32_ipv4" {
  value = [
    "20.42.7.0/25",
    "52.149.248.0/28",
    "52.149.248.64/27",
    "52.149.248.96/28",
    "52.154.68.16/28",
    "52.154.68.32/28",
    "52.170.171.192/28",
    "52.170.171.240/28",
    "52.186.36.16/28",
  ]
}
output "AzureEventGrid_region32_ipv6" {
  value = [
    "2603:1030:210:1::380/121",
  ]
}
output "AzureEventGrid_region32" {
  value = [
    "20.42.7.0/25",
    "52.149.248.0/28",
    "52.149.248.64/27",
    "52.149.248.96/28",
    "52.154.68.16/28",
    "52.154.68.32/28",
    "52.170.171.192/28",
    "52.170.171.240/28",
    "52.186.36.16/28",
    "2603:1030:210:1::380/121",
  ]
}
output "AzureIoTHub_region32_ipv4" {
  value = [
    "13.82.93.138/32",
    "20.49.109.128/25",
    "20.49.110.0/26",
    "20.49.110.128/25",
    "40.71.14.128/25",
    "40.76.71.185/32",
    "40.78.229.128/25",
    "40.79.156.128/25",
    "40.114.53.146/32",
    "52.168.180.95/32",
    "104.211.18.153/32",
    "137.117.83.38/32",
    "168.61.54.255/32",
  ]
}
output "AzureIoTHub_region32_ipv6" {
  value = [
    "2603:1030:210:402::300/123",
    "2603:1030:210:802::240/123",
    "2603:1030:210:c02::240/123",
  ]
}
output "AzureIoTHub_region32" {
  value = [
    "13.82.93.138/32",
    "20.49.109.128/25",
    "20.49.110.0/26",
    "20.49.110.128/25",
    "40.71.14.128/25",
    "40.76.71.185/32",
    "40.78.229.128/25",
    "40.79.156.128/25",
    "40.114.53.146/32",
    "52.168.180.95/32",
    "104.211.18.153/32",
    "137.117.83.38/32",
    "168.61.54.255/32",
    "2603:1030:210:402::300/123",
    "2603:1030:210:802::240/123",
    "2603:1030:210:c02::240/123",
  ]
}
output "AzureKeyVault_region32_ipv4" {
  value = [
    "20.42.64.44/30",
    "20.42.73.8/30",
    "20.62.134.76/30",
    "20.62.134.224/29",
    "20.88.156.160/29",
    "20.185.217.251/32",
    "20.185.218.1/32",
    "40.71.10.200/30",
    "40.76.196.75/32",
    "40.76.212.37/32",
    "40.85.185.208/32",
    "40.87.69.184/32",
    "40.117.157.122/32",
    "52.168.109.101/32",
    "52.255.152.252/32",
    "52.255.152.255/32",
    "137.116.120.244/32",
  ]
}
output "AzureKeyVault_region32_ipv6" {
  value = [
    "2603:1030:210::340/125",
    "2603:1030:210:402::80/125",
    "2603:1030:210:802::80/125",
    "2603:1030:210:c02::80/125",
  ]
}
output "AzureKeyVault_region32" {
  value = [
    "20.42.64.44/30",
    "20.42.73.8/30",
    "20.62.134.76/30",
    "20.62.134.224/29",
    "20.88.156.160/29",
    "20.185.217.251/32",
    "20.185.218.1/32",
    "40.71.10.200/30",
    "40.76.196.75/32",
    "40.76.212.37/32",
    "40.85.185.208/32",
    "40.87.69.184/32",
    "40.117.157.122/32",
    "52.168.109.101/32",
    "52.255.152.252/32",
    "52.255.152.255/32",
    "137.116.120.244/32",
    "2603:1030:210::340/125",
    "2603:1030:210:402::80/125",
    "2603:1030:210:802::80/125",
    "2603:1030:210:c02::80/125",
  ]
}
output "AzureMachineLearning_region32_ipv4" {
  value = [
    "20.42.0.240/28",
    "20.62.135.208/28",
    "40.71.11.64/28",
    "40.78.227.32/28",
    "40.79.154.64/28",
    "52.255.214.109/32",
    "52.255.217.127/32",
  ]
}
output "AzureMachineLearning_region32_ipv6" {
  value = [
    "2603:1030:210:1::2c0/122",
  ]
}
output "AzureMachineLearning_region32" {
  value = [
    "20.42.0.240/28",
    "20.62.135.208/28",
    "40.71.11.64/28",
    "40.78.227.32/28",
    "40.79.154.64/28",
    "52.255.214.109/32",
    "52.255.217.127/32",
    "2603:1030:210:1::2c0/122",
  ]
}
output "AzureMonitor_region32_ipv4" {
  value = [
    "13.82.100.176/32",
    "13.82.184.151/32",
    "13.90.93.206/32",
    "13.90.248.141/32",
    "13.90.249.229/32",
    "13.90.251.123/32",
    "13.92.40.198/32",
    "13.92.40.223/32",
    "13.92.138.16/32",
    "13.92.179.52/32",
    "13.92.211.249/32",
    "13.92.232.146/32",
    "13.92.254.218/32",
    "13.92.255.146/32",
    "20.42.0.68/31",
    "20.42.65.72/29",
    "20.42.65.128/25",
    "20.42.73.16/29",
    "20.42.73.128/25",
    "20.49.109.46/31",
    "20.49.109.80/28",
    "20.49.111.16/28",
    "20.49.111.32/28",
    "20.62.132.0/25",
    "23.96.28.38/32",
    "40.71.12.224/28",
    "40.71.12.240/30",
    "40.71.12.248/29",
    "40.71.13.168/29",
    "40.71.14.112/30",
    "40.71.183.225/32",
    "40.76.29.55/32",
    "40.76.53.225/32",
    "40.78.226.216/29",
    "40.78.229.32/29",
    "40.79.154.80/29",
    "40.79.156.32/29",
    "40.85.180.90/32",
    "40.87.67.118/32",
    "40.112.49.101/32",
    "40.117.80.207/32",
    "40.117.95.162/32",
    "40.117.147.74/32",
    "40.117.190.239/32",
    "40.117.197.224/32",
    "40.121.57.2/32",
    "40.121.61.208/32",
    "40.121.135.131/32",
    "40.121.163.228/32",
    "40.121.165.150/32",
    "40.121.210.163/32",
    "52.150.36.187/32",
    "52.168.112.64/32",
    "52.168.116.72/29",
    "52.168.136.177/32",
    "52.186.121.41/32",
    "52.186.126.31/32",
    "52.188.179.229/32",
    "52.191.197.52/32",
    "52.224.125.230/32",
    "52.224.162.220/32",
    "52.224.235.3/32",
    "52.226.151.250/32",
    "104.41.152.101/32",
    "104.41.157.59/32",
    "104.45.136.42/32",
    "168.62.169.17/32",
  ]
}
output "AzureMonitor_region32_ipv6" {
  value = [
    "2603:1030:210::60/123",
    "2603:1030:210::1c0/122",
    "2603:1030:210::300/123",
    "2603:1030:210::360/123",
    "2603:1030:210::500/121",
    "2603:1030:210:1::280/122",
    "2603:1030:210:402::500/121",
    "2a01:111:f100:2000::a83e:3015/128",
    "2a01:111:f100:2000::a83e:301c/128",
    "2a01:111:f100:2000::a83e:3022/128",
    "2a01:111:f100:2000::a83e:3039/128",
    "2a01:111:f100:2000::a83e:3070/128",
    "2a01:111:f100:2000::a83e:3083/128",
    "2a01:111:f100:2000::a83e:3097/128",
    "2a01:111:f100:2000::a83e:30a9/128",
    "2a01:111:f100:2000::a83e:30f3/128",
    "2a01:111:f100:2000::a83e:313a/128",
    "2a01:111:f100:2000::a83e:313d/128",
    "2a01:111:f100:2000::a83e:32a5/128",
    "2a01:111:f100:2000::a83e:3348/128",
    "2a01:111:f100:2000::a83e:335c/128",
    "2a01:111:f100:2000::a83e:336c/128",
    "2a01:111:f100:2000::a83e:3370/128",
    "2a01:111:f100:2000::a83e:337e/128",
    "2a01:111:f100:2000::a83e:33ad/128",
    "2a01:111:f100:2000::a83e:33bf/128",
    "2a01:111:f100:2000::a83e:3649/128",
    "2a01:111:f100:2002::8975:2c8c/128",
    "2a01:111:f100:2002::8975:2c8e/128",
    "2a01:111:f100:2002::8975:2ce6/128",
    "2a01:111:f100:2002::8975:2d44/128",
    "2a01:111:f100:2002::8975:2d6a/128",
    "2a01:111:f100:2002::8975:2e91/128",
    "2a01:111:f100:2002::8975:2eaa/128",
    "2a01:111:f100:2002::8975:2fa3/128",
    "2a01:111:f100:2002::8975:2fac/128",
    "2a01:111:f100:2002::8975:2fc3/128",
  ]
}
output "AzureMonitor_region32" {
  value = [
    "13.82.100.176/32",
    "13.82.184.151/32",
    "13.90.93.206/32",
    "13.90.248.141/32",
    "13.90.249.229/32",
    "13.90.251.123/32",
    "13.92.40.198/32",
    "13.92.40.223/32",
    "13.92.138.16/32",
    "13.92.179.52/32",
    "13.92.211.249/32",
    "13.92.232.146/32",
    "13.92.254.218/32",
    "13.92.255.146/32",
    "20.42.0.68/31",
    "20.42.65.72/29",
    "20.42.65.128/25",
    "20.42.73.16/29",
    "20.42.73.128/25",
    "20.49.109.46/31",
    "20.49.109.80/28",
    "20.49.111.16/28",
    "20.49.111.32/28",
    "20.62.132.0/25",
    "23.96.28.38/32",
    "40.71.12.224/28",
    "40.71.12.240/30",
    "40.71.12.248/29",
    "40.71.13.168/29",
    "40.71.14.112/30",
    "40.71.183.225/32",
    "40.76.29.55/32",
    "40.76.53.225/32",
    "40.78.226.216/29",
    "40.78.229.32/29",
    "40.79.154.80/29",
    "40.79.156.32/29",
    "40.85.180.90/32",
    "40.87.67.118/32",
    "40.112.49.101/32",
    "40.117.80.207/32",
    "40.117.95.162/32",
    "40.117.147.74/32",
    "40.117.190.239/32",
    "40.117.197.224/32",
    "40.121.57.2/32",
    "40.121.61.208/32",
    "40.121.135.131/32",
    "40.121.163.228/32",
    "40.121.165.150/32",
    "40.121.210.163/32",
    "52.150.36.187/32",
    "52.168.112.64/32",
    "52.168.116.72/29",
    "52.168.136.177/32",
    "52.186.121.41/32",
    "52.186.126.31/32",
    "52.188.179.229/32",
    "52.191.197.52/32",
    "52.224.125.230/32",
    "52.224.162.220/32",
    "52.224.235.3/32",
    "52.226.151.250/32",
    "104.41.152.101/32",
    "104.41.157.59/32",
    "104.45.136.42/32",
    "168.62.169.17/32",
    "2603:1030:210::60/123",
    "2603:1030:210::1c0/122",
    "2603:1030:210::300/123",
    "2603:1030:210::360/123",
    "2603:1030:210::500/121",
    "2603:1030:210:1::280/122",
    "2603:1030:210:402::500/121",
    "2a01:111:f100:2000::a83e:3015/128",
    "2a01:111:f100:2000::a83e:301c/128",
    "2a01:111:f100:2000::a83e:3022/128",
    "2a01:111:f100:2000::a83e:3039/128",
    "2a01:111:f100:2000::a83e:3070/128",
    "2a01:111:f100:2000::a83e:3083/128",
    "2a01:111:f100:2000::a83e:3097/128",
    "2a01:111:f100:2000::a83e:30a9/128",
    "2a01:111:f100:2000::a83e:30f3/128",
    "2a01:111:f100:2000::a83e:313a/128",
    "2a01:111:f100:2000::a83e:313d/128",
    "2a01:111:f100:2000::a83e:32a5/128",
    "2a01:111:f100:2000::a83e:3348/128",
    "2a01:111:f100:2000::a83e:335c/128",
    "2a01:111:f100:2000::a83e:336c/128",
    "2a01:111:f100:2000::a83e:3370/128",
    "2a01:111:f100:2000::a83e:337e/128",
    "2a01:111:f100:2000::a83e:33ad/128",
    "2a01:111:f100:2000::a83e:33bf/128",
    "2a01:111:f100:2000::a83e:3649/128",
    "2a01:111:f100:2002::8975:2c8c/128",
    "2a01:111:f100:2002::8975:2c8e/128",
    "2a01:111:f100:2002::8975:2ce6/128",
    "2a01:111:f100:2002::8975:2d44/128",
    "2a01:111:f100:2002::8975:2d6a/128",
    "2a01:111:f100:2002::8975:2e91/128",
    "2a01:111:f100:2002::8975:2eaa/128",
    "2a01:111:f100:2002::8975:2fa3/128",
    "2a01:111:f100:2002::8975:2fac/128",
    "2a01:111:f100:2002::8975:2fc3/128",
  ]
}
output "AzureOpenDatasets_region32_ipv4" {
  value = [
    "20.42.4.224/28",
  ]
}
output "AzureOpenDatasets_region32_ipv6" {
  value = [
  ]
}
output "AzureOpenDatasets_region32" {
  value = [
    "20.42.4.224/28",
  ]
}
output "AzurePortal_region32_ipv4" {
  value = [
    "13.68.130.251/32",
    "13.68.235.98/32",
    "13.90.156.71/32",
    "13.92.138.76/32",
    "20.42.6.192/27",
    "20.49.109.36/30",
    "20.49.109.44/31",
    "20.49.109.48/28",
    "20.62.128.240/29",
    "40.71.15.144/28",
    "40.114.78.132/32",
    "40.117.86.243/32",
    "40.117.237.78/32",
  ]
}
output "AzurePortal_region32_ipv6" {
  value = [
    "2603:1030:210::100/121",
    "2603:1030:210:1::680/121",
  ]
}
output "AzurePortal_region32" {
  value = [
    "13.68.130.251/32",
    "13.68.235.98/32",
    "13.90.156.71/32",
    "13.92.138.76/32",
    "20.42.6.192/27",
    "20.49.109.36/30",
    "20.49.109.44/31",
    "20.49.109.48/28",
    "20.62.128.240/29",
    "40.71.15.144/28",
    "40.114.78.132/32",
    "40.117.86.243/32",
    "40.117.237.78/32",
    "2603:1030:210::100/121",
    "2603:1030:210:1::680/121",
  ]
}
output "AzureResourceManager_region32_ipv4" {
  value = [
    "20.62.130.0/23",
    "40.71.13.224/28",
    "40.79.158.0/23",
  ]
}
output "AzureResourceManager_region32_ipv6" {
  value = [
    "2603:1030:210::180/122",
    "2603:1030:210:402::280/122",
  ]
}
output "AzureResourceManager_region32" {
  value = [
    "20.62.130.0/23",
    "40.71.13.224/28",
    "40.79.158.0/23",
    "2603:1030:210::180/122",
    "2603:1030:210:402::280/122",
  ]
}
output "AzureSecurityCenter_region32_ipv4" {
  value = [
    "52.168.112.96/27",
  ]
}
output "AzureSecurityCenter_region32_ipv6" {
  value = [
  ]
}
output "AzureSecurityCenter_region32" {
  value = [
    "52.168.112.96/27",
  ]
}
output "AzureSignalR_region32_ipv4" {
  value = [
    "20.42.64.128/25",
    "20.42.72.0/25",
    "20.62.133.64/27",
    "20.88.155.0/25",
    "40.71.15.0/25",
  ]
}
output "AzureSignalR_region32_ipv6" {
  value = [
    "2603:1030:210:5::/120",
  ]
}
output "AzureSignalR_region32" {
  value = [
    "20.42.64.128/25",
    "20.42.72.0/25",
    "20.62.133.64/27",
    "20.88.155.0/25",
    "40.71.15.0/25",
    "2603:1030:210:5::/120",
  ]
}
output "AzureUpdateDelivery_region32_ipv4" {
  value = [
    "20.42.24.29/32",
    "20.42.24.50/32",
    "20.62.190.184/29",
    "20.185.109.208/32",
    "20.185.214.153/32",
    "40.90.247.210/32",
    "52.142.21.136/29",
    "104.45.177.233/32",
  ]
}
output "AzureUpdateDelivery_region32_ipv6" {
  value = [
    "2603:1030:20e:3::2a0/125",
  ]
}
output "AzureUpdateDelivery_region32" {
  value = [
    "20.42.24.29/32",
    "20.42.24.50/32",
    "20.62.190.184/29",
    "20.185.109.208/32",
    "20.185.214.153/32",
    "40.90.247.210/32",
    "52.142.21.136/29",
    "104.45.177.233/32",
    "2603:1030:20e:3::2a0/125",
  ]
}
output "BatchNodeManagement_region32_ipv4" {
  value = [
    "13.92.114.103/32",
    "20.42.6.224/27",
    "23.96.12.112/32",
    "23.96.101.73/32",
    "23.96.109.140/32",
    "40.71.12.192/27",
    "40.78.227.0/27",
    "40.79.154.32/27",
    "40.88.48.36/32",
    "52.188.222.115/32",
    "104.41.129.99/32",
    "137.117.45.176/32",
    "137.117.109.143/32",
    "168.62.36.128/32",
    "168.62.168.27/32",
    "191.236.37.239/32",
    "191.236.38.142/32",
  ]
}
output "BatchNodeManagement_region32_ipv6" {
  value = [
    "2603:1030:210:1::340/122",
  ]
}
output "BatchNodeManagement_region32" {
  value = [
    "13.92.114.103/32",
    "20.42.6.224/27",
    "23.96.12.112/32",
    "23.96.101.73/32",
    "23.96.109.140/32",
    "40.71.12.192/27",
    "40.78.227.0/27",
    "40.79.154.32/27",
    "40.88.48.36/32",
    "52.188.222.115/32",
    "104.41.129.99/32",
    "137.117.45.176/32",
    "137.117.109.143/32",
    "168.62.36.128/32",
    "168.62.168.27/32",
    "191.236.37.239/32",
    "191.236.38.142/32",
    "2603:1030:210:1::340/122",
  ]
}
output "DataFactory_region32_ipv4" {
  value = [
    "20.42.2.0/23",
    "20.42.4.0/26",
    "20.42.64.0/28",
    "20.49.111.0/29",
    "40.71.14.32/28",
    "40.78.229.96/28",
  ]
}
output "DataFactory_region32_ipv6" {
  value = [
    "2603:1030:210:1::480/121",
    "2603:1030:210:1::500/122",
    "2603:1030:210:1::700/121",
    "2603:1030:210:1::780/122",
    "2603:1030:210:402::330/124",
    "2603:1030:210:802::210/124",
    "2603:1030:210:c02::210/124",
  ]
}
output "DataFactory_region32" {
  value = [
    "20.42.2.0/23",
    "20.42.4.0/26",
    "20.42.64.0/28",
    "20.49.111.0/29",
    "40.71.14.32/28",
    "40.78.229.96/28",
    "2603:1030:210:1::480/121",
    "2603:1030:210:1::500/122",
    "2603:1030:210:1::700/121",
    "2603:1030:210:1::780/122",
    "2603:1030:210:402::330/124",
    "2603:1030:210:802::210/124",
    "2603:1030:210:c02::210/124",
  ]
}
output "AzureEventHub_region32_ipv4" {
  value = [
    "13.90.83.7/32",
    "13.90.208.184/32",
    "13.92.124.151/32",
    "13.92.180.208/32",
    "13.92.190.184/32",
    "20.42.68.64/26",
    "20.42.74.0/26",
    "20.88.153.0/26",
    "40.71.10.128/26",
    "40.71.100.98/32",
    "40.76.29.197/32",
    "40.76.40.11/32",
    "40.76.194.119/32",
    "40.78.226.128/26",
    "40.79.155.0/26",
    "40.117.88.66/32",
    "40.121.84.50/32",
    "40.121.141.232/32",
    "40.121.148.193/32",
    "52.168.14.144/32",
    "52.168.66.180/32",
    "52.168.117.0/26",
    "52.168.146.69/32",
    "52.168.147.11/32",
    "52.179.6.240/32",
    "52.179.8.35/32",
    "52.191.213.188/32",
    "52.191.228.245/32",
    "52.226.36.235/32",
    "104.45.135.34/32",
    "104.45.147.24/32",
    "137.117.85.236/32",
    "137.117.89.253/32",
    "137.117.91.152/32",
    "137.135.102.226/32",
    "168.62.52.235/32",
    "191.236.32.73/32",
    "191.236.32.191/32",
    "191.236.35.225/32",
    "191.237.47.93/32",
  ]
}
output "AzureEventHub_region32_ipv6" {
  value = [
    "2603:1030:210:1::240/122",
    "2603:1030:210:402::1c0/123",
    "2603:1030:210:802::160/123",
    "2603:1030:210:c02::160/123",
    "2603:10e1:100:2::34bf:e4f5/128",
  ]
}
output "AzureEventHub_region32" {
  value = [
    "13.90.83.7/32",
    "13.90.208.184/32",
    "13.92.124.151/32",
    "13.92.180.208/32",
    "13.92.190.184/32",
    "20.42.68.64/26",
    "20.42.74.0/26",
    "20.88.153.0/26",
    "40.71.10.128/26",
    "40.71.100.98/32",
    "40.76.29.197/32",
    "40.76.40.11/32",
    "40.76.194.119/32",
    "40.78.226.128/26",
    "40.79.155.0/26",
    "40.117.88.66/32",
    "40.121.84.50/32",
    "40.121.141.232/32",
    "40.121.148.193/32",
    "52.168.14.144/32",
    "52.168.66.180/32",
    "52.168.117.0/26",
    "52.168.146.69/32",
    "52.168.147.11/32",
    "52.179.6.240/32",
    "52.179.8.35/32",
    "52.191.213.188/32",
    "52.191.228.245/32",
    "52.226.36.235/32",
    "104.45.135.34/32",
    "104.45.147.24/32",
    "137.117.85.236/32",
    "137.117.89.253/32",
    "137.117.91.152/32",
    "137.135.102.226/32",
    "168.62.52.235/32",
    "191.236.32.73/32",
    "191.236.32.191/32",
    "191.236.35.225/32",
    "191.237.47.93/32",
    "2603:1030:210:1::240/122",
    "2603:1030:210:402::1c0/123",
    "2603:1030:210:802::160/123",
    "2603:1030:210:c02::160/123",
    "2603:10e1:100:2::34bf:e4f5/128",
  ]
}
output "GatewayManager_region32_ipv4" {
  value = [
    "13.92.84.128/32",
    "20.42.0.72/29",
    "20.88.159.0/27",
    "40.71.11.96/29",
    "40.88.222.179/32",
    "40.88.223.53/32",
    "52.179.10.142/32",
  ]
}
output "GatewayManager_region32_ipv6" {
  value = [
    "2603:1030:210:1::40/122",
  ]
}
output "GatewayManager_region32" {
  value = [
    "13.92.84.128/32",
    "20.42.0.72/29",
    "20.88.159.0/27",
    "40.71.11.96/29",
    "40.88.222.179/32",
    "40.88.223.53/32",
    "52.179.10.142/32",
    "2603:1030:210:1::40/122",
  ]
}
output "HDInsight_region32_ipv4" {
  value = [
    "13.82.225.233/32",
    "40.71.13.160/29",
    "40.71.175.99/32",
    "52.146.79.136/29",
    "168.61.48.131/32",
    "168.61.49.99/32",
  ]
}
output "HDInsight_region32_ipv6" {
  value = [
    "2603:1030:210:402::320/124",
  ]
}
output "HDInsight_region32" {
  value = [
    "13.82.225.233/32",
    "40.71.13.160/29",
    "40.71.175.99/32",
    "52.146.79.136/29",
    "168.61.48.131/32",
    "168.61.49.99/32",
    "2603:1030:210:402::320/124",
  ]
}
output "LogicApps_region32_ipv4" {
  value = [
    "13.92.98.111/32",
    "20.42.64.48/28",
    "20.42.72.160/27",
    "23.100.29.190/32",
    "23.101.132.208/32",
    "23.101.136.201/32",
    "23.101.139.153/32",
    "40.114.8.21/32",
    "40.114.12.31/32",
    "40.114.13.216/32",
    "40.114.14.143/32",
    "40.114.40.186/32",
    "40.114.51.5/32",
    "40.114.82.191/32",
    "40.117.99.79/32",
    "40.117.100.228/32",
    "40.121.91.41/32",
    "104.45.153.81/32",
    "137.116.126.165/32",
    "137.117.72.32/32",
    "137.135.106.54/32",
    "191.238.41.107/32",
  ]
}
output "LogicApps_region32_ipv6" {
  value = [
    "2603:1030:210:402::3c0/124",
    "2603:1030:210:402::3e0/123",
  ]
}
output "LogicApps_region32" {
  value = [
    "13.92.98.111/32",
    "20.42.64.48/28",
    "20.42.72.160/27",
    "23.100.29.190/32",
    "23.101.132.208/32",
    "23.101.136.201/32",
    "23.101.139.153/32",
    "40.114.8.21/32",
    "40.114.12.31/32",
    "40.114.13.216/32",
    "40.114.14.143/32",
    "40.114.40.186/32",
    "40.114.51.5/32",
    "40.114.82.191/32",
    "40.117.99.79/32",
    "40.117.100.228/32",
    "40.121.91.41/32",
    "104.45.153.81/32",
    "137.116.126.165/32",
    "137.117.72.32/32",
    "137.135.106.54/32",
    "191.238.41.107/32",
    "2603:1030:210:402::3c0/124",
    "2603:1030:210:402::3e0/123",
  ]
}
output "MicrosoftCloudAppSecurity_region32_ipv4" {
  value = [
    "20.42.29.162/32",
    "20.42.31.48/32",
    "20.42.31.251/32",
    "40.71.14.16/28",
    "40.76.78.217/32",
    "40.78.229.64/28",
    "40.79.156.112/28",
    "40.114.112.147/32",
    "40.121.134.1/32",
    "52.151.237.243/32",
    "52.151.238.5/32",
    "52.151.244.65/32",
    "52.151.247.27/32",
    "52.188.217.236/32",
    "52.190.26.220/32",
    "52.190.31.62/32",
    "52.191.237.188/32",
    "52.191.238.65/32",
    "52.224.188.157/32",
    "52.224.188.168/32",
    "52.224.190.225/32",
    "52.224.191.62/32",
    "52.224.201.216/32",
    "52.224.201.223/32",
    "52.224.202.86/32",
    "52.224.202.91/32",
    "104.45.168.103/32",
    "104.45.168.104/32",
    "104.45.168.106/32",
    "104.45.168.108/32",
    "104.45.168.111/32",
    "104.45.168.114/32",
    "104.45.170.70/32",
    "104.45.170.127/32",
    "104.45.170.161/32",
    "104.45.170.173/32",
    "104.45.170.174/31",
    "104.45.170.176/32",
    "104.45.170.178/32",
    "104.45.170.180/32",
    "104.45.170.182/31",
    "104.45.170.184/31",
    "104.45.170.186/32",
    "104.45.170.188/32",
    "104.45.170.191/32",
    "104.45.170.194/32",
    "104.45.170.196/32",
    "104.211.9.226/32",
  ]
}
output "MicrosoftCloudAppSecurity_region32_ipv6" {
  value = [
  ]
}
output "MicrosoftCloudAppSecurity_region32" {
  value = [
    "20.42.29.162/32",
    "20.42.31.48/32",
    "20.42.31.251/32",
    "40.71.14.16/28",
    "40.76.78.217/32",
    "40.78.229.64/28",
    "40.79.156.112/28",
    "40.114.112.147/32",
    "40.121.134.1/32",
    "52.151.237.243/32",
    "52.151.238.5/32",
    "52.151.244.65/32",
    "52.151.247.27/32",
    "52.188.217.236/32",
    "52.190.26.220/32",
    "52.190.31.62/32",
    "52.191.237.188/32",
    "52.191.238.65/32",
    "52.224.188.157/32",
    "52.224.188.168/32",
    "52.224.190.225/32",
    "52.224.191.62/32",
    "52.224.201.216/32",
    "52.224.201.223/32",
    "52.224.202.86/32",
    "52.224.202.91/32",
    "104.45.168.103/32",
    "104.45.168.104/32",
    "104.45.168.106/32",
    "104.45.168.108/32",
    "104.45.168.111/32",
    "104.45.168.114/32",
    "104.45.170.70/32",
    "104.45.170.127/32",
    "104.45.170.161/32",
    "104.45.170.173/32",
    "104.45.170.174/31",
    "104.45.170.176/32",
    "104.45.170.178/32",
    "104.45.170.180/32",
    "104.45.170.182/31",
    "104.45.170.184/31",
    "104.45.170.186/32",
    "104.45.170.188/32",
    "104.45.170.191/32",
    "104.45.170.194/32",
    "104.45.170.196/32",
    "104.211.9.226/32",
  ]
}
output "MicrosoftContainerRegistry_region32_ipv4" {
  value = [
    "40.71.10.208/29",
    "40.78.226.200/29",
    "40.79.154.96/29",
  ]
}
output "MicrosoftContainerRegistry_region32_ipv6" {
  value = [
    "2603:1030:210:402::88/125",
    "2603:1030:210:802::88/125",
    "2603:1030:210:c02::88/125",
  ]
}
output "MicrosoftContainerRegistry_region32" {
  value = [
    "40.71.10.208/29",
    "40.78.226.200/29",
    "40.79.154.96/29",
    "2603:1030:210:402::88/125",
    "2603:1030:210:802::88/125",
    "2603:1030:210:c02::88/125",
  ]
}
output "PowerQueryOnline_region32_ipv4" {
  value = [
    "20.42.4.200/30",
  ]
}
output "PowerQueryOnline_region32_ipv6" {
  value = [
    "2603:1030:210:1::200/123",
  ]
}
output "PowerQueryOnline_region32" {
  value = [
    "20.42.4.200/30",
    "2603:1030:210:1::200/123",
  ]
}
output "SCCservice_region32_ipv4" {
  value = [
    "13.72.73.110/32",
    "13.90.86.1/32",
    "13.92.97.243/32",
    "13.92.188.209/32",
    "13.92.190.185/32",
    "40.71.86.107/32",
    "40.117.35.99/32",
    "40.121.214.58/32",
    "52.168.88.247/32",
    "52.168.89.30/32",
    "52.168.92.234/32",
    "52.168.116.0/26",
    "52.168.136.186/32",
    "52.168.139.96/32",
    "52.168.141.90/32",
    "52.168.143.85/32",
    "52.168.168.165/32",
    "52.168.178.77/32",
    "52.168.179.117/32",
    "52.168.180.168/32",
    "52.170.28.184/32",
    "52.170.34.217/32",
    "52.170.37.236/32",
    "52.170.209.22/32",
    "52.179.23.200/32",
    "137.117.96.184/32",
    "137.117.97.51/32",
  ]
}
output "SCCservice_region32_ipv6" {
  value = [
  ]
}
output "SCCservice_region32" {
  value = [
    "13.72.73.110/32",
    "13.90.86.1/32",
    "13.92.97.243/32",
    "13.92.188.209/32",
    "13.92.190.185/32",
    "40.71.86.107/32",
    "40.117.35.99/32",
    "40.121.214.58/32",
    "52.168.88.247/32",
    "52.168.89.30/32",
    "52.168.92.234/32",
    "52.168.116.0/26",
    "52.168.136.186/32",
    "52.168.139.96/32",
    "52.168.141.90/32",
    "52.168.143.85/32",
    "52.168.168.165/32",
    "52.168.178.77/32",
    "52.168.179.117/32",
    "52.168.180.168/32",
    "52.170.28.184/32",
    "52.170.34.217/32",
    "52.170.37.236/32",
    "52.170.209.22/32",
    "52.179.23.200/32",
    "137.117.96.184/32",
    "137.117.97.51/32",
  ]
}
output "AzureServiceBus_region32_ipv4" {
  value = [
    "20.42.65.0/26",
    "20.42.68.0/26",
    "20.42.72.192/26",
    "20.42.73.64/26",
    "20.88.153.64/26",
    "40.71.10.192/29",
    "40.78.226.192/29",
    "40.79.154.88/29",
    "40.114.86.33/32",
    "52.168.29.86/32",
    "52.168.112.128/26",
    "52.168.116.192/26",
  ]
}
output "AzureServiceBus_region32_ipv6" {
  value = [
    "2603:1030:210:1::220/123",
    "2603:1030:210:402::170/125",
    "2603:1030:210:802::150/125",
    "2603:1030:210:c02::150/125",
  ]
}
output "AzureServiceBus_region32" {
  value = [
    "20.42.65.0/26",
    "20.42.68.0/26",
    "20.42.72.192/26",
    "20.42.73.64/26",
    "20.88.153.64/26",
    "40.71.10.192/29",
    "40.78.226.192/29",
    "40.79.154.88/29",
    "40.114.86.33/32",
    "52.168.29.86/32",
    "52.168.112.128/26",
    "52.168.116.192/26",
    "2603:1030:210:1::220/123",
    "2603:1030:210:402::170/125",
    "2603:1030:210:802::150/125",
    "2603:1030:210:c02::150/125",
  ]
}
output "ServiceFabric_region32_ipv4" {
  value = [
    "13.92.124.124/32",
    "20.42.64.40/30",
    "20.42.72.132/30",
    "40.71.11.104/29",
    "40.76.203.148/32",
    "40.76.205.181/32",
  ]
}
output "ServiceFabric_region32_ipv6" {
  value = [
    "2603:1030:210:402::98/125",
    "2603:1030:210:802::98/125",
    "2603:1030:210:c02::98/125",
  ]
}
output "ServiceFabric_region32" {
  value = [
    "13.92.124.124/32",
    "20.42.64.40/30",
    "20.42.72.132/30",
    "40.71.11.104/29",
    "40.76.203.148/32",
    "40.76.205.181/32",
    "2603:1030:210:402::98/125",
    "2603:1030:210:802::98/125",
    "2603:1030:210:c02::98/125",
  ]
}
output "AzureSQL_region32_ipv4" {
  value = [
    "20.42.65.64/29",
    "20.42.65.96/27",
    "20.42.68.192/27",
    "20.42.69.0/25",
    "20.42.69.128/26",
    "20.42.73.0/29",
    "20.42.73.32/27",
    "20.42.74.192/27",
    "20.42.75.0/25",
    "20.42.75.128/26",
    "20.62.132.160/27",
    "20.62.132.192/27",
    "20.62.133.0/26",
    "23.96.89.109/32",
    "23.96.106.191/32",
    "40.71.8.0/26",
    "40.71.8.192/26",
    "40.71.9.0/26",
    "40.71.9.192/26",
    "40.71.83.113/32",
    "40.76.2.172/32",
    "40.76.26.90/32",
    "40.76.42.44/32",
    "40.76.65.222/32",
    "40.76.66.9/32",
    "40.76.193.221/32",
    "40.78.224.0/26",
    "40.78.224.128/26",
    "40.78.225.0/26",
    "40.78.225.128/26",
    "40.79.152.0/26",
    "40.79.152.192/26",
    "40.79.153.0/26",
    "40.79.153.192/26",
    "40.114.45.195/32",
    "40.114.81.142/32",
    "40.117.42.73/32",
    "40.117.44.71/32",
    "40.121.143.204/32",
    "40.121.149.49/32",
    "40.121.158.30/32",
    "52.168.116.64/29",
    "52.168.117.96/27",
    "52.168.117.128/27",
    "52.168.117.160/29",
    "52.168.117.192/26",
    "52.168.118.0/25",
    "52.168.166.153/32",
    "52.170.98.29/32",
    "104.41.152.74/32",
    "104.45.158.30/32",
    "191.238.6.43/32",
    "191.238.6.44/31",
    "191.238.6.46/32",
  ]
}
output "AzureSQL_region32_ipv6" {
  value = [
    "2603:1030:210::320/123",
    "2603:1030:210::380/121",
    "2603:1030:210:400::/123",
    "2603:1030:210:401::/123",
    "2603:1030:210:800::/123",
    "2603:1030:210:801::/123",
    "2603:1030:210:c00::/123",
    "2603:1030:210:c01::/123",
  ]
}
output "AzureSQL_region32" {
  value = [
    "20.42.65.64/29",
    "20.42.65.96/27",
    "20.42.68.192/27",
    "20.42.69.0/25",
    "20.42.69.128/26",
    "20.42.73.0/29",
    "20.42.73.32/27",
    "20.42.74.192/27",
    "20.42.75.0/25",
    "20.42.75.128/26",
    "20.62.132.160/27",
    "20.62.132.192/27",
    "20.62.133.0/26",
    "23.96.89.109/32",
    "23.96.106.191/32",
    "40.71.8.0/26",
    "40.71.8.192/26",
    "40.71.9.0/26",
    "40.71.9.192/26",
    "40.71.83.113/32",
    "40.76.2.172/32",
    "40.76.26.90/32",
    "40.76.42.44/32",
    "40.76.65.222/32",
    "40.76.66.9/32",
    "40.76.193.221/32",
    "40.78.224.0/26",
    "40.78.224.128/26",
    "40.78.225.0/26",
    "40.78.225.128/26",
    "40.79.152.0/26",
    "40.79.152.192/26",
    "40.79.153.0/26",
    "40.79.153.192/26",
    "40.114.45.195/32",
    "40.114.81.142/32",
    "40.117.42.73/32",
    "40.117.44.71/32",
    "40.121.143.204/32",
    "40.121.149.49/32",
    "40.121.158.30/32",
    "52.168.116.64/29",
    "52.168.117.96/27",
    "52.168.117.128/27",
    "52.168.117.160/29",
    "52.168.117.192/26",
    "52.168.118.0/25",
    "52.168.166.153/32",
    "52.170.98.29/32",
    "104.41.152.74/32",
    "104.45.158.30/32",
    "191.238.6.43/32",
    "191.238.6.44/31",
    "191.238.6.46/32",
    "2603:1030:210::320/123",
    "2603:1030:210::380/121",
    "2603:1030:210:400::/123",
    "2603:1030:210:401::/123",
    "2603:1030:210:800::/123",
    "2603:1030:210:801::/123",
    "2603:1030:210:c00::/123",
    "2603:1030:210:c01::/123",
  ]
}
output "AzureStorage_region32_ipv4" {
  value = [
    "13.68.163.32/28",
    "13.68.165.64/28",
    "13.68.167.240/28",
    "13.82.33.32/28",
    "13.82.152.16/28",
    "13.82.152.48/28",
    "13.82.152.80/28",
    "20.38.98.0/24",
    "20.47.1.0/24",
    "20.47.16.0/23",
    "20.47.31.0/24",
    "20.60.0.0/24",
    "20.60.2.0/23",
    "20.60.6.0/23",
    "20.60.60.0/22",
    "20.60.128.0/23",
    "20.60.134.0/23",
    "20.60.146.0/23",
    "20.60.220.0/23",
    "20.150.32.0/23",
    "20.150.90.0/24",
    "20.157.39.0/24",
    "20.157.59.0/24",
    "20.157.132.0/24",
    "20.157.147.0/24",
    "20.209.0.0/23",
    "23.96.64.64/26",
    "40.71.104.16/28",
    "40.71.104.32/28",
    "40.71.240.16/28",
    "40.117.48.80/28",
    "40.117.48.112/28",
    "40.117.104.16/28",
    "52.179.24.16/28",
    "52.186.112.32/27",
    "52.226.8.32/27",
    "52.226.8.80/28",
    "52.226.8.96/28",
    "52.226.8.128/27",
    "52.234.176.48/28",
    "52.234.176.64/28",
    "52.234.176.96/27",
    "52.239.152.0/22",
    "52.239.168.0/22",
    "52.239.207.192/26",
    "52.239.214.0/23",
    "52.239.220.0/23",
    "52.239.246.0/23",
    "52.239.252.0/24",
    "52.240.48.16/28",
    "52.240.48.32/28",
    "52.240.60.16/28",
    "52.240.60.32/28",
    "52.240.60.64/27",
    "138.91.96.64/26",
    "138.91.96.128/26",
    "168.62.32.0/26",
    "168.62.32.192/26",
    "168.62.33.128/26",
    "191.237.32.128/28",
    "191.237.32.208/28",
    "191.237.32.240/28",
    "191.238.0.0/26",
    "191.238.0.224/28",
  ]
}
output "AzureStorage_region32_ipv6" {
  value = [
  ]
}
output "AzureStorage_region32" {
  value = [
    "13.68.163.32/28",
    "13.68.165.64/28",
    "13.68.167.240/28",
    "13.82.33.32/28",
    "13.82.152.16/28",
    "13.82.152.48/28",
    "13.82.152.80/28",
    "20.38.98.0/24",
    "20.47.1.0/24",
    "20.47.16.0/23",
    "20.47.31.0/24",
    "20.60.0.0/24",
    "20.60.2.0/23",
    "20.60.6.0/23",
    "20.60.60.0/22",
    "20.60.128.0/23",
    "20.60.134.0/23",
    "20.60.146.0/23",
    "20.60.220.0/23",
    "20.150.32.0/23",
    "20.150.90.0/24",
    "20.157.39.0/24",
    "20.157.59.0/24",
    "20.157.132.0/24",
    "20.157.147.0/24",
    "20.209.0.0/23",
    "23.96.64.64/26",
    "40.71.104.16/28",
    "40.71.104.32/28",
    "40.71.240.16/28",
    "40.117.48.80/28",
    "40.117.48.112/28",
    "40.117.104.16/28",
    "52.179.24.16/28",
    "52.186.112.32/27",
    "52.226.8.32/27",
    "52.226.8.80/28",
    "52.226.8.96/28",
    "52.226.8.128/27",
    "52.234.176.48/28",
    "52.234.176.64/28",
    "52.234.176.96/27",
    "52.239.152.0/22",
    "52.239.168.0/22",
    "52.239.207.192/26",
    "52.239.214.0/23",
    "52.239.220.0/23",
    "52.239.246.0/23",
    "52.239.252.0/24",
    "52.240.48.16/28",
    "52.240.48.32/28",
    "52.240.60.16/28",
    "52.240.60.32/28",
    "52.240.60.64/27",
    "138.91.96.64/26",
    "138.91.96.128/26",
    "168.62.32.0/26",
    "168.62.32.192/26",
    "168.62.33.128/26",
    "191.237.32.128/28",
    "191.237.32.208/28",
    "191.237.32.240/28",
    "191.238.0.0/26",
    "191.238.0.224/28",
  ]
}
output "StorageSyncService_region32_ipv4" {
  value = [
    "20.42.4.248/29",
    "104.41.148.238/32",
  ]
}
output "StorageSyncService_region32_ipv6" {
  value = [
    "2603:1030:210:1::300/123",
  ]
}
output "StorageSyncService_region32" {
  value = [
    "20.42.4.248/29",
    "104.41.148.238/32",
    "2603:1030:210:1::300/123",
  ]
}
